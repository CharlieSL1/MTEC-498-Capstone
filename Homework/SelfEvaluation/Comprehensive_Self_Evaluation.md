...
- **Week 10**: Matched the vision module with Ableton Live via the custom `PanGu.amxd` Max for Live device, enabling direct DAW control from gestures
...
**Current Achievement**: Functional gesture recognition system with OSC communication driving the `PanGu.amxd` Max for Live device, delivering end-to-end gesture-to-Ableton spatial audio control.
...
- **Max for Live**: Authored the `PanGu.amxd` device that maps OSC gesture IDs to Ableton Live macros and multichannel routing
...
- **PanGu.amxd Max for Live device** exporting multi-channel `mc.matrix~` routing, `live.numbox`/`live.grid` UI, and gesture-triggered `groove~` looping for Ableton Live
...
- Synchronizing OSC data with Ableton Live via Max for Live while keeping latency performance-ready
...
- ✅ **Week 10**: Ableton Live integration using the `PanGu.amxd` Max for Live audio effect
- 🔄 **Ongoing**: Performance rehearsals and spatial audio refinement for the live set
...
- Authored `PanGu.amxd`, a Max for Live audio effect that translates OSC gesture IDs into Live-friendly controls
...
- **Live Integration**: Devised mapping logic so depth-camera gestures can automate Ableton Live macros without race conditions
...
- **Deliverable Packaging**: Exported `PanGu.amxd` Max for Live device and documented installation steps for grading
...
5. **Ableton Live Handshake**: Translating OSC gestures into dependable Max for Live controls without MIDI feedback loops
...
- **Pipeline Instrumentation**: Logged OSC payloads and Live parameter responses to align gesture events with M4L automation
...
- **Live Systems Integration**: Coordinated rehearsable handoffs between Python, Max/MSP, and Ableton Live
...
- **DAW Automation**: Max for Live development skills translate to commercial music tech workflows
...
- **Full Performance Prep**: Rehearse an 8-minute set where depth-camera gestures drive Ableton Live via `PanGu.amxd`
- **Spatial Routing Polish**: Tune Max/MSP and Live macros for multi-speaker placement and Ableton Session automation
- **Gesture Recognition Refinement**: Improve accuracy and add more gesture types
- **User Interface Development**: Create performance interface for live use with clear visual feedback
...
- **Final Target Performance**: Stage a complete gesture-driven spatial set using the depth camera + Ableton Live rig
